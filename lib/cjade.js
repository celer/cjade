...
var cjade=function(srcDir,compileDir,baseDir){
  ...
  var compileDir=compileDir||null;
	...
	var cjadeJS = path.resolve("../static/cjade.js");
	var runtimeJS = path.resolve("../node_modules/jade/runtime.min.js");
	var cjadeMinJS = path.resolve("../temp/_cjade.min.js");
	      	...
	      	res.sendfile(cjadeMinJS);
					...
					fs.readFile(cjadeJS,function(err,cjadeJS){
						fs.readFile(runtimeJS,function(err,runtimeJS){
							if(!err){
								var min = uglificate(cjadeJS.toString()+runtimeJS.toString());	
							...
							}	else { res.send("Error reading cjade.js",500); }
						});
					});
				}
			});
